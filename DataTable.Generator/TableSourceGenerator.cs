...
namespace DataTable.Generator
{
    ...
    public sealed class TableSourceGenerator : IIncrementalGenerator
    {
        ...
        private static void EmitDatabase(SourceProductionContext context, ImmutableArray<TableIdentity> identities)
        {
            ...
            {
                context.AddSource(
                    "DataTable.Runtime.TableDatabase.g.cs",
        ...
        private static bool IsRequiredIdentityProperty(IPropertySymbol property, INamedTypeSymbol rowType)
        {
            ...
            return property.Type is INamedTypeSymbol idType &&
                   ...
                   idType.Name == "Id" &&
                   idType.Arity == 1 &&
                   idType.ContainingNamespace.ToDisplayString() == "DataTable.Primitives" &&
        ...
        private static TypeConversion? GetConversion(ITypeSymbol type)
        {
            if (type is INamedTypeSymbol named && named.IsGenericType &&
                ...
                named.ContainingNamespace.ToDisplayString() == "DataTable.Primitives")
            {
            ...
            if (type.Name == "AssetAddress" && type.ContainingNamespace.ToDisplayString() == "DataTable.Primitives")
            {
                return new TypeConversion(SqliteStorageKind.RequiredText, "{0}.Value", "new global::DataTable.Primitives.AssetAddress({0})");
            }
                ...
                nullable.TypeArguments.Length == 1)
            {
                ...
                if (underlyingType is INamedTypeSymbol nullableId && nullableId.IsGenericType &&
                    ...
                    nullableId.ContainingNamespace.ToDisplayString() == "DataTable.Primitives")
                {
                ...
                if (underlyingType.Name == "AssetAddress" &&
                    underlyingType.ContainingNamespace.ToDisplayString() == "DataTable.Primitives")
                {
                    return new TypeConversion(
                        SqliteStorageKind.NullableText,
                        "{0}.HasValue ? {0}.Value.Value : null",
                        "{0} == null ? (global::DataTable.Primitives.AssetAddress?)null : new global::DataTable.Primitives.AssetAddress({0}!)");
                }
            }
        ...
        private static string RenderDatabaseFile(IReadOnlyList<TableIdentity> identities)
        {
            ...
            builder.AppendLine();
            builder.AppendLine("namespace DataTable.Runtime");
            builder.AppendLine("{");
        ...
        private static void RenderDatabase(StringBuilder builder, IReadOnlyList<TableIdentity> identities)
        {
            ...
            builder.AppendLine("        {");
            builder.AppendLine("            return InitializeAsync(dbPath, global::DataTable.Runtime.TableDatabaseOptions.Direct);");
            builder.AppendLine("        }");
            builder.AppendLine();
            builder.AppendLine("        public async global::System.Threading.Tasks.Task InitializeAsync(string dbPath, global::DataTable.Runtime.TableDatabaseOptions options)");
            builder.AppendLine("        {");
            ...
            builder.AppendLine("            if (options.LoadMode != global::DataTable.Runtime.TableLoadMode.Direct && options.LoadMode != global::DataTable.Runtime.TableLoadMode.Preload)");
            ...
            builder.AppendLine("            }");
            builder.AppendLine();
            builder.AppendLine("            if (options.LoadMode == global::DataTable.Runtime.TableLoadMode.Preload)");
            builder.AppendLine("            {");
            builder.AppendLine("                try");
            builder.AppendLine("                {");
        ...
        private static void RenderTable(StringBuilder builder, TableModel model)
        {
            ...
            builder.AppendLine("        {");
            builder.Append("            global::DataTable.Runtime.Internal.SqliteRuntime.ValidateTable(database, \"")
            ...
            {
                builder.Append("            global::DataTable.Runtime.Internal.SqliteRuntime.ValidateIndex(database, \"")
            ...
            builder.Append("            var statement = global::DataTable.Runtime.Internal.SqliteRuntime.Prepare(database, \"SELECT ");
            for (var i = 0; i < model.Members.Count; i++)
            {
                if (i > 0) builder.Append(", ");
                builder.Append("\\\"").Append(EscapeString(model.Members[i].Name)).Append("\\\"");
            ...
            builder.AppendLine("                    throw global::DataTable.Runtime.Internal.SqliteRuntime.CreateException(database, \"preload table\", stepResult);");
            builder.AppendLine("                }");
            builder.AppendLine("            }");
            builder.AppendLine("            finally");
            builder.AppendLine("            {");
        ...
        private static void RenderFinder(StringBuilder builder, TableModel model, FinderModel finder)
        {
            ...
            builder.Append("            var statement = global::DataTable.Runtime.Internal.SqliteRuntime.Prepare(connection.Handle, \"SELECT ");
            for (var i = 0; i < model.Members.Count; i++)
            {
                if (i > 0) builder.Append(", ");
                builder.Append("\\\"").Append(EscapeString(model.Members[i].Name)).Append("\\\"");
            ...
            for (var i = 0; i < finder.Members.Count; i++)
            {
                builder.Append("                global::DataTable.Runtime.Internal.SqliteRuntime.")
            ...
            if (finder.FindAll)
            {
                ...
                builder.AppendLine("                    }");
                builder.AppendLine("                    if (stepResult == global::SQLitePCL.raw.SQLITE_DONE) break;");
                builder.AppendLine("                    throw global::DataTable.Runtime.Internal.SqliteRuntime.CreateException(connection.Handle, \"step query\", stepResult);");
                builder.AppendLine("                }");
            }
            else
            {
        ...
        {
            return "DataTable.Runtime." + model.TableName + "Table.g.cs";
        }
        ...
        private static string GetReadExpression(SqliteStorageKind storageKind, string statementName, int columnIndex)
        {
            ...
            return "global::DataTable.Runtime.Internal.SqliteRuntime." + methodName + "(" + statementName + ", " + columnIndex + ")";
        }
