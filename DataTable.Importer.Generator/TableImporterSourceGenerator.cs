...
namespace DataTable.Importer.Generator
{
    [Generator]
    public sealed class TableImporterSourceGenerator : IIncrementalGenerator
    {
        ...
        private const string FindAllByAttributeName = "DataTable.Annotations.FindAllByAttribute";
        private const string PrimitivesNamespace = "DataTable.Primitives";
            ...
            context.RegisterSourceOutput(analysis, static (sourceContext, result) =>
            {
                ...
                foreach (var table in result.Tables)
                {
                    sourceContext.AddSource(
                        "DataTable.Importer.Generated." + table.RowName + "SheetImporter.g.cs",
                        SourceText.From(RenderTable(table), Encoding.UTF8));
                }

                sourceContext.AddSource(
                    "DataTable.Importer.Generated.TableImporterCatalog.g.cs",
        ...
        private static string RenderCatalog(ImmutableArray<TableModel> tables)
        {
            ...
            builder.AppendLine();
            builder.AppendLine("namespace DataTable.Importer.Generated");
            builder.AppendLine("{");
            builder.AppendLine("    internal static class GeneratedTableImporterCatalog");
            builder.AppendLine("    {");
            builder.AppendLine("        public static global::System.Collections.Generic.IReadOnlyList<global::DataTable.Importer.Runtime.ITableSheetImporter> All { get; } =");
            builder.AppendLine("            new global::DataTable.Importer.Runtime.ITableSheetImporter[]");
            builder.AppendLine("            {");
        ...
        private static string RenderTable(TableModel table)
        {
            ...
            builder.Append("    internal sealed class ").Append(table.RowName)
                .AppendLine("SheetImporter : global::DataTable.Importer.Runtime.ITableSheetImporter");
            builder.AppendLine("    {");
        ...
        private static void RenderImport(StringBuilder builder, TableModel table)
        {
            ...
            builder.AppendLine("        {");
            builder.Append("            var columns = global::DataTable.Importer.Runtime.ExcelImportRuntime.ReadHeader(reader, TableName, new[] { ");
            ...
            builder.AppendLine(" });");
            builder.Append("            var statement = global::DataTable.Importer.Runtime.SqliteWriteRuntime.Prepare(connection.Handle, \"")
            ...
            builder.AppendLine("                    if (global::DataTable.Importer.Runtime.ExcelImportRuntime.IsBlankRow(reader, columns))");
            builder.AppendLine("                        continue;");
            for (var i = 0; i < table.Members.Count; i++)
                RenderBind(builder, table, table.Members[i], i);
            builder.AppendLine("                    global::DataTable.Importer.Runtime.SqliteWriteRuntime.Step(connection.Handle, statement);");
            builder.AppendLine("                    global::DataTable.Importer.Runtime.SqliteWriteRuntime.Reset(connection.Handle, statement);");
            builder.AppendLine("                    importedRowCount++;");
            builder.AppendLine("                }");
            builder.AppendLine("            }");
            builder.AppendLine("            finally");
            builder.AppendLine("            {");
            builder.AppendLine("                global::DataTable.Importer.Runtime.SqliteWriteRuntime.Finalize(statement);");
            builder.AppendLine("            }");
        ...
        private static void RenderBind(StringBuilder builder, TableModel table, MemberModel member, int zeroBasedIndex)
        {
            ...
            switch (member.Conversion.ValueKind)
            {
                case ValueKind.Enum:
                    ...
                    break;
                case ValueKind.Real:
                    readerExpression = "global::DataTable.Importer.Runtime.ExcelImportRuntime.ReadDouble(" + value + context + ")";
                    bindMethod = "BindDouble";
                    break;
                case ValueKind.String:
                    readerExpression = "global::DataTable.Importer.Runtime.ExcelImportRuntime." +
                ...
                case ValueKind.DateTime:
                    readerExpression = "global::DataTable.Importer.Runtime.ExcelImportRuntime.ReadDateTimeTicks(" + value + context + ")";
                    bindMethod = "BindInt64";
                    break;
                case ValueKind.DateTimeOffset:
                    readerExpression = "global::DataTable.Importer.Runtime.ExcelImportRuntime.ReadDateTimeOffsetTicks(" + value + context + ")";
                    bindMethod = "BindInt64";
                    break;
                case ValueKind.TimeSpan:
                    readerExpression = "global::DataTable.Importer.Runtime.ExcelImportRuntime.ReadTimeSpanTicks(" + value + context + ")";
                    bindMethod = "BindInt64";
                    break;
                case ValueKind.Guid:
                    readerExpression = "global::DataTable.Importer.Runtime.ExcelImportRuntime.ReadGuid(" + value + context + ")";
                    bindMethod = "BindText";
                    break;
                default:
            ...
            builder.Append("                    global::DataTable.Importer.Runtime.SqliteWriteRuntime.")
        ...
        private static void RenderEnumReaders(StringBuilder builder, TableModel table)
        {
            foreach (var member in table.Members.Where(static member => member.Conversion.ValueKind == ValueKind.Enum))
            {
                ...
                builder.AppendLine("        {");
                builder.AppendLine("            if (global::DataTable.Importer.Runtime.ExcelImportRuntime.TryReadEnumName(value, out var name))");
                builder.AppendLine("            {");
                ...
                builder.AppendLine("                    default: throw global::DataTable.Importer.Runtime.ExcelImportRuntime.InvalidEnum(");
                builder.Append("                        TableName, \"").Append(Escape(member.Name)).Append("\", rowNumber, value, \"")
                    .Append(Escape(enumModel.Name)).AppendLine("\");");
                builder.AppendLine("                }");
                builder.AppendLine("            }");
                builder.Append("            return global::DataTable.Importer.Runtime.ExcelImportRuntime.ReadInt64(value, TableName, \"")
